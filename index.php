...
function insertDettes(array &$tabClients,array $tabDette,$index ):void{
 ...
 function listerDettesByClient(string $numero,array $tabDette):void{
    foreach ($tabDette as $dette) {
        ...
        echo "Montant versé de la dette : ". $dette["montverse"]."\t";
    }
 }

 function uniqref( array $clients,array $dette,string $ref):int{
    foreach ($dette["paiement"] as $index => $paiement) {
         if ($paiement["ref"] == $ref) {
            return $index;
         }
     }
 ...
 function montantverse(array $paiement){
    do{
        $montant= montantverse($paiement);
        if ($montant > $dette["montdette"]) {
            echo "Montant de paiement superieur au montant de la dette \n";
        } else {
            $dette["montdette"] -= $montant;
            $dette["paiement"][]=[
                "ref"=>uniqref(),
                "date"=>date("d-m-Y"),
                "montantpaie"=>$montant
            ];
            echo "Paiement effectue avec succes \n";
            echo "Montant restant de la dette : ". $dette["montdette"]."\n";
        }
     } while (verifMontant($montant));
    }

 
 function payerDette($dette,$paiement,$montant){
